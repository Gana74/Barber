...
2. Бот покажет список активных записей (первые 50)
...
12345 — Мужская стрижка 15.01.2025 14:00-15:00 — Иван Петров (+79991234567)
12346 — Оформление бороды 15.01.2025 16:00-16:30 — Сергей Сидоров (+79997654321)
12347 — Мужская стрижка 16.01.2025 10:00-11:00 — Алексей Иванов (+79991112233)
12348 — Стрижка под машинку 16.01.2025 12:00-12:30 — Дмитрий Козлов (+79994445566)
12349 — Мужская стрижка 17.01.2025 15:00-16:00 — Максим Смирнов (+79997778899)
```
...
- В уведомлении менеджеру при создании новой записи
...
- Попросить клиента написать боту `@userinfobot` — он покажет свой ID
...
Таблица находится по адресу, который предоставит администратор. Обычно это ссылка вида:
...
2. Найдите запись:
   ...
   - В боте: "Просмотр записей" → найдите по имени/телефону → запомните ID
   - В Google Sheets: лист "Записи" → найдите по имени/телефону → запомните ID

3. Отмените запись:

   - В боте: "Отменить запись (по ID)" → введите ID
   - Или: "Отменить запись (по коду)" → введите код отмены
...
Вы: [Открываете бота → "Просмотр записей" → находите запись]
    [Нажимаете "Отменить запись (по ID)" → вводите ID: 12345]

Бот: Запись 12345 отменена.
...
1. Найдите ID записи (через "Просмотр записей" или в Google Sheets)
2. Отмените запись через бота: "Отменить запись (по ID)" → введите ID
...
1. **Через бота:**
   ...
   - ID указан в начале каждой строки (например: `12345 — Мужская стрижка...`)
...
Если вы начали какое-то действие (например, ввод ID для отмены записи) и хотите отменить:
...
```
Вы: Отменить запись (по ID)

Бот: Отправьте ID записи, которую нужно отменить.
     Для отмены напишите /admin_cancel
